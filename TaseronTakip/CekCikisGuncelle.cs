...
{
    public partial class CekCikisGuncelle : Form
    {
        public CekCikisGuncelle()
        {
            InitializeComponent();
        }
        ...
        private void CekIslemGuncelle()
        {
            ...
            {
                OleDbCommand comK = new OleDbCommand("update CekIslem set DuzenlemeTarihi=@DT,TedarikciID=@F,Aciklama=@Aciklama,CekNo=@CN,Banka=@B,Sube=@S,HesapNo=@HN,VadeTarihi=@VT,Cikis=@Cikis,KasaID=@KID where ((CekIslem.CekID)=@ID)", con);
                ...
                comK.Parameters.AddWithValue("@VT", Cekvadetarihi.Value);
                comK.Parameters.AddWithValue("@Cikis", Cektutar.Text);
                ...
                try
                {
                    if (comK.ExecuteNonQuery() > 0)
                    {
                        if (EskiCariID != YeniCariID)
                        {
                            ...
                            comEski.Parameters.AddWithValue("@FT", Cektutar.Text);
        ...
        {
            OleDbCommand com = new OleDbCommand("SELECT CekIslem.DuzenlemeTarihi, CekIslem.TedarikciID, CekIslem.Aciklama, CekIslem.CekNo, CekIslem.Banka, CekIslem.Sube, CekIslem.HesapNo, CekIslem.VadeTarihi, CekIslem.Cikis, CekIslem.KasaID FROM CekIslem WHERE (((CekIslem.CekID)=@ID))", con);
        ...
        {
            string Aciklamam = "Çek Verildi (Çek No : " + ceknumarasi.Text + " - Ödeme Tarihi : " + Cekvadetarihi.Value + ")";
            ...
            comK.Parameters.AddWithValue("@A", Aciklamam);
        ...
        private void CekCikisGuncelle_Load(object sender, EventArgs e)
        {
            ID = Convert.ToInt32(CekCikis.ID);
            GiderKasa();
            TedarikciGel();
            GelBilgi();
        }
