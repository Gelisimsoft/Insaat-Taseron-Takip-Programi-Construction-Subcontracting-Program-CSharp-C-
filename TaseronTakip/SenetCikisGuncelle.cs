...
{
    public partial class SenetCikisGuncelle : Form
    {
        public SenetCikisGuncelle()
        {
            InitializeComponent();
        }
        ...
        {
            OleDbCommand com = new OleDbCommand("SELECT SenetIslem.DuzenlemeTarihi, SenetIslem.TedarikciID, SenetIslem.Aciklama, SenetIslem.SenetNo, SenetIslem.VadeTarihi, SenetIslem.Cikis, SenetIslem.KasaID FROM SenetIslem WHERE (((SenetIslem.SenetID)=@ID))", con);
        ...
        {
            string Aciklamam = "Senet Verildi (Senet No : " + Senetnumarasi.Text + " - Ödeme Tarihi : " + Senetvadetarihi.Value + ")";
        ...
        private void SenetIslemGuncelle()
        {
            ...
            {
                OleDbCommand comK = new OleDbCommand("update SenetIslem set DuzenlemeTarihi=@DT,TedarikciID=@F,Aciklama=@Aciklama,SenetNo=@SN,VadeTarihi=@VT,Cikis=@Cikis,KasaID=@KID where SenetIslem.SenetID=@ID", con);
                ...
                comK.Parameters.AddWithValue("@VT", Senetvadetarihi.Value);
                comK.Parameters.AddWithValue("@Cikis", Senettutar.Text);
        ...
        private void SenetCikisGuncelle_Load(object sender, EventArgs e)
        {
            ID = Convert.ToInt32(SenetCikis.ID);
            TedarikciGel();
            GiderKasa();
            GelBilgi();
        }
